...
def GetTorsionQuartet01(mol):
    # for i, mol in enumerate(list(mols.values())):
    idx = []
    matched_TP = []
    matched_TS = []
    for HC in root.findall("hierarchyClass"):
        if not HC.get("name") == "GG": # <hierarchyClass>
            for HSC in HC: # <hierarchySubClass>
                for TR in HSC.iter("torsionRule"):
                # for TR in HC.iter("torsionRule"): # iter can span multiple sub-part in hierarchies
                    TS = TR.get("smarts")
                    matched_TP,matched_TS, idx = get_matches_pairs_torsionsmarts_idx(mol, TS, matched_TP, matched_TS, idx)
    for TR in root.find("hierarchyClass[@name='GG']").iter("torsionRule"): # no HSC in GG HC
        TS = TR.get("smarts")
        matched_TP,matched_TS, idx = get_matches_pairs_torsionsmarts_idx(mol, TS, matched_TP, matched_TS, idx)
    return matched_TP,matched_TS, idx
...
class TorsionFragmentGenerator_TL(object):
    def __init__(self, mol, outpath, name, rmsd, numConfs):
        ...
        TorsionPairs, TorsionSmarts, TorsionQuartets = GetTorsionQuartet01(mol)
        print(">>> TorsionPairs:")
        print(TorsionPairs)
        print(">>> TorsionSmarts:")
        print(TorsionSmarts)
